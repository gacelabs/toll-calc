function autocomplete(inp, arr, index) {
	...
	inp.addEventListener("input", function (e) {
		...
		arr.forEach(function (matchValue) {
			var sValue = matchValue[index];
			/*check if the item starts with the same letters as the text field value:*/
			// if (sValue.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
			// console.log(sValue, val);
			var valLength = sValue.toUpperCase().indexOf(val.toUpperCase());
			if (valLength >= 0) {
				// console.log(sValue);
				/*create a DIV element for each matching element:*/
				b = document.createElement("DIV");
				/*make the matching letters bold:*/
				// b.innerHTML = "<strong>" + sValue.substr(valLength, val.length) + "</strong>";
				// b.innerHTML += sValue.substr(val.length);
				b.innerHTML = sValue.replace(sValue.substr(valLength, val.length), "<strong>" + sValue.substr(valLength, val.length) + "</strong>");
				/*insert a input field that will hold the current array item's value:*/
				b.innerHTML += "<input type='hidden' value='" + sValue.trim() + "'>";
				/*execute a function when someone clicks on the item value (DIV element):*/
				b.addEventListener("click", function (e) {
					/*insert the value for the autocomplete text field:*/
					inp.value = this.getElementsByTagName("input")[0].value;
					inp.dataset.set = JSON.stringify(matchValue);
